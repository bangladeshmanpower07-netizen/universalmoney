...
const UserSchema = new Schema({
  ...
  // Everyone (admin, user, merchant) logs in with a password now. Admins use email + password;
  // users/merchants use phone + password.
...
async function verifyPassword(pw, hash) { return hash ? bcrypt.compare(pw, hash) : false; }
 ...
 * login-phone, login-password, register-phone, register-otp, register-info, register-password,
 * forgot-otp, forgot-password, admin.
 * Users/merchants authenticate with phone + password. Admins authenticate with email + password.
 */
...
app.post('/login/check-phone', authLimiter, csrfGuard, body('phone').trim().notEmpty(), async (req, res) => {
  ...
  res.render('login', await renderData(req, 'login-password', { phoneMasked: maskPhone(phone), rawPhone: phone }));
});

app.post('/login/verify-password', authLimiter, csrfGuard, body('password').notEmpty(), async (req, res) => {
  const phone = req.session.auth && req.session.auth.phone;
  if (!phone) return res.redirect('/login');
  const userDoc = await User.findOne({ phone, role: { $in: ['user', 'merchant'] } });
  if (!userDoc) return res.redirect('/login');
  if (userDoc.lock_until && userDoc.lock_until > new Date()) {
    return res.status(423).render('login', await renderData(req, 'login-password', { phoneMasked: maskPhone(phone), rawPhone: phone, error: 'Account temporarily locked. Try again later.' }));
  }
  ...
  if (!ok) {
    ...
    await userDoc.save();
    return res.status(401).render('login', await renderData(req, 'login-password', { phoneMasked: maskPhone(phone), rawPhone: phone, error: 'Incorrect password.' }));
  }
...
/* ---------- REGISTER: phone -> OTP -> info -> password ---------- */
...
app.post('/register/info', csrfGuard,
  ...
  async (req, res) => {
    ...
    res.render('login', await renderData(req, 'register-password', {}));
  });

app.post('/register/set-password', csrfGuard, body('password').isLength({ min: 8, max: 128 }), body('confirm_password').notEmpty(), async (req, res) => {
  const pending = req.session.auth;
  if (!pending || pending.step !== 'password') return res.redirect('/register');
  if (req.body.password !== req.body.confirm_password) {
    return res.status(400).render('login', await renderData(req, 'register-password', { error: 'Passwords do not match.' }));
  }
  ...
  const userDoc = await User.create({
    ...
    password_hash, phone_verified: true, role: pending.account_type, status: 'active',
  });
...
/* ---------- FORGOT PASSWORD (phone accounts): phone -> OTP -> new password ---------- */
app.post('/forgot-password/send-otp', authLimiter, csrfGuard, body('phone').trim().notEmpty(), async (req, res) => {
  const phone = normalizePhone(req.body.phone);
...
app.post('/forgot-password/resend-otp', authLimiter, csrfGuard, async (req, res) => {
  const phone = req.session.auth && req.session.auth.phone;
  if (!phone) return res.redirect('/login');
  const otp = await issueOtp(phone, 'forgot_pin');
...
app.post('/forgot-password/verify-otp', authLimiter, csrfGuard, body('otp').isLength({ min: 4, max: 4 }), async (req, res) => {
  const phone = req.session.auth && req.session.auth.phone;
  if (!phone) return res.redirect('/login');
  ...
  req.session.auth = { phone, step: 'forgot-password' };
  res.render('login', await renderData(req, 'forgot-password', {}));
});

app.post('/forgot-password/reset', csrfGuard, body('password').isLength({ min: 8, max: 128 }), body('confirm_password').notEmpty(), async (req, res) => {
  const pending = req.session.auth;
  if (!pending || pending.step !== 'forgot-password') return res.redirect('/login');
  if (req.body.password !== req.body.confirm_password) {
    return res.status(400).render('login', await renderData(req, 'forgot-password', { error: 'Passwords do not match.' }));
  }
  const userDoc = await User.findOne({ phone: pending.phone, role: { $in: ['user', 'merchant'] } });
  if (userDoc) {
    userDoc.password_hash = await hashPassword(req.body.password);
    userDoc.failed_login_attempts = 0;
    userDoc.lock_until = null;
    await userDoc.save();
    await notify(userDoc._id, userDoc.role, 'Security', 'Password Reset', 'Your login password was reset successfully. If this was not you, contact support.');
    await auditLog(req, 'password_reset', 'User', userDoc._id);
  }
  req.session.auth = null;
  res.render('login', await renderData(req, 'login-phone', { notice: 'Password reset successful. Please sign in with your new password.' }));
});
...
// Everyone (admin, user, merchant) changes their login password here.
